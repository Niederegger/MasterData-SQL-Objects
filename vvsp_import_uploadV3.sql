...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[vvsp_import_uploadV3]') )
  drop procedure dbo.vvsp_import_uploadV3
go

create procedure dbo.vvsp_import_uploadV3
  ...
  @SOURCEFILE   varchar(256),  -- Wenn der Wert aus einer Datei stammt, der tatsächliche Dateiname, zB "File 20170426_Frankfurt_Data.csv"
...
insert vv_log 
  select @now,						-- LOG_TIMESTAMP
         ...
         'vvsp_import_uploadV3',	-- LOG_PROGRAM_NAME
...
begin tran
  ...
  insert vv_uploads 
        (UPL_UPLOAD_ID, UPL_SOURCE_ID, UPL_USER_ID, UPL_HOST_NAME, UPL_TIMESTAMP, UPL_RECEIVED_ROWS, UPL_IDENTICAL_ROWS, UPL_EXISTING_ROWS, UPL_INSERTED_ROWS, UPL_DATA_ORIGIN, UPL_URLSOURCE, UPL_SOURCEFILE, UPL_COMMENT)
  select @upload_id,  @SOURCE_ID, user_name(), HOST_NAME(), @now, @received_rows, @identical_rows, @existing_rows, @inserted_rows, @DATA_ORIGIN, @URLSOURCE, @SOURCEFILE, @COMMENT  
  ...
  select getdate(),                         -- LOG_TIMESTAMP
         user_name(),                       -- LOG_USER_ID
         HOST_NAME(),                       -- LOG_HOST_NAME, 
         'vvsp_import_uploadV3',            -- LOG_PROGRAM_NAME, ist varchar(32) in vv_LOG
...
exec vvsp_import_uploadV3 'DBAG', 'x', 'x', 'x', 'x'
